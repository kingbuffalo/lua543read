 ...
 */
/*
 * 遍历到一个临时table变量时，把这个table放到正确的队列中去。当且仅当object在这次遍历中被标记时返回tru.
 * 在 propagation阶段 把table放到grayagain列表中,在aotmic阶段时会再被访问一次。
 * 如果是白白标记，会被临时变量集合中被再次访问
 * 如果是白标记，则会在atomic阶段中清除。
 * 在分代模式下，一些tabel会保存在gray列表中以便 post-processing.这个操作是在genlink进行的
 */
 ...
 */
/*
 * 扫描thread 标记当前栈到栈顶的元素，在最后的扫描阶段 清除其它栈位置的元素
 * 这样保证stack有（有效）的object
 * Threads没有栅栏.
 * 在分代模式下，旧threads会在每个gc cycle中被访问.因为它们有可能会指向新的object
 * 在步进模式下, threads 会有可能在cycle结束之前被修改。为了保证这些访问，
 * 在他们不是新的时候（分代下),或在propagate阶段下(在步进模式)
 * threads会放进gray list中去
 */
 ...
 */
/*
 * 当上一个是一个坏回收的时候，进行一个大的回收
 *
 * 当一个程序比较大时，它申请了很内存，但是 增加得非常小
 * 在别的场景中，分代模式做小回收时会浪费时间,大回收又会变得太频繁。而每次回收都只是回收一点点object
 * 为了减少分代与步进两种切换的消耗.回收器会在一个bad collection后 完成一个步进的垃圾回收。 同时只有在完成一个good collection后(上一次只遍历了9/8的对象)，才切换成分代模式
 *
 * 回收器需要在sweeping之前选择到底是维持步进模式还是切换回分代模式，在这个时间点上，它并不知道到底有多少内存在使用，所以它不能根据内存使用的大小来决定是不是切换回分代模式。而是使用他遍历了多少对象(atomic后得出)作为一个代理。
 * g->lastatomic用来保存上一次的内存回收数据
 * g->lastatomic !=0 时表示上一次的回收是坏的
 *
 */
...
static lu_mem atomic (lua_State *L) {
	...
	g->gcstate = GCSatomic;
	markobject(g, L);  /* mark running thread */ //标记运行的线程
	/* registry and global metatables may be changed by API */ 
	// registry 和 全局元表，可能会被修改
	markvalue(g, &g->l_registry);
	markmt(g);  /* mark global metatables */ //标记全局的metatable
	work += propagateall(g);  /* empties 'gray' list */ //清空gray万晴
	/* remark occasional upvalues of (maybe) dead threads */
	//重新标记 死线程的upvalues 
	work += remarkupvals(g);
	work += propagateall(g);  /* propagate changes */
	g->gray = grayagain;
	work += propagateall(g);  /* traverse 'grayagain' list */ //访问 grayagain列表
